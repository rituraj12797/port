---
title: "Approaching 50Million Transaction/S"
date: '2026-2-2'
...
TThe system is closely coupled, with each component pinned to a different CPU core. These components/threads communicate with each other via this lock-free queue.

Notice how this system is using 12 of these LFQs ( Green One's represent bi directional lock free queues which are implemented by using 2 unidirectional ones' internally)? Say if data needs to travel from Comp 6 --> Comp 3 ---> Comp 2 ---> Comp 1 ----> Comp 6; it needs to go through 4 threads, and in total 4 LFQs. Now, the total time for the data to just get inside these queues and then get popped out from them will be = `(read_latency + write_latency)`.
...
Since we use this in both likely your read and write, the latency multiplies by 2, and sometimes by 3, simply due to the time spent on computation.
...
2. **Ping Pong Effect:** The bus snooping problem, and how at each step, one process cancels the validity of the other.


## How to Optimize

### 1. The Wrap-Around Logic (Replacng Modulo)
...
**Step 2:** This invalidate signal is picked up by Core 2. It sees its 2nd L1 line contains this variable, so it invalidates this **whole** cache line. Remember that `next_index_to_write` also lied on this line! So because the *line* is invalid, that variable's cache is also invalidated.
...
But There's Is one Major Win Which The benchmark Does Not Show.
...
Before you assume I code using AI let me make it clear, I make a lot of mis-spells so for that obvious reason I have used Agentic-AI to write gramatically correct comments(only) for me, code is purely written by me.
...
You CPU is a very Smart Gambler(Like Hakari from JJK.) when it comes to predicting outcomes of branch instructions thanks to it's Hardware Branch Pridicting Unit.
